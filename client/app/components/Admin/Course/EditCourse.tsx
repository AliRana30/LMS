...
import { useEditCourseMutation, useGetCoursesQuery } from '@/redux/features/courses/coursesApi'
...
const EditCourse : FC<Props> = ({id}) => {
  ...
  const handleCourseCreate = async () => {
        ...
        await editCourse({courseId : id , courseData : data } );
    }
  ...
  useEffect(()=>{
    if(editCourseData){
        ...
        setBenefits(editCourseData.benefits.length > 0 ? editCourseData.benefits : [{ title: "" }])
        setPrerequisites(editCourseData.prerequisites.length > 0 ? editCourseData.prerequisites : [{ title: "" }])
        setCourseContentData(
          editCourseData.courseData && editCourseData.courseData.length > 0 
            ? editCourseData.courseData.map((content: any) => ({
                videoUrl: content.videoUrl || "",
                title: content.title || "",
                description: content.description || "",
                videoSection: content.videoSection || "Untitled Section",
                links: content.links && content.links.length > 0 ? content.links : [{ title: "", url: "" }],
                suggestions: content.suggestions || ""
              }))
            : [{
                videoUrl: "",
  ...
  useEffect(() => {
    ...
    if (error) {
      if ("data" in error) {
        ...
        toast.error(errorMessage.data.message || "Failed to update course. Please try again.")
      }
    }
  }, [isSuccess, error])
          ...
          <div className="flex-1 w-full order-2 lg:order-1">
            <div className={`rounded-2xl shadow-2xl backdrop-blur-sm transition-all duration-500 border ${theme === 'dark'
              ...
              <div className="p-4 sm:p-6 lg:p-8">
                ...
                {active === 3 && <CoursePreview courseData={courseData} active={active} setActive={setActive} handleCourseCreate={handleCourseCreate} isEdit={true}/>}
              </div>
            </div>
          </div>
